...
resource "aws_route_table" "ap-northeast-1-private" {
  vpc_id = aws_vpc.main_vpc.id
...
resource "aws_route_table_association" "ap-northeast-1-private" {
  subnet_id      = aws_subnet.ap-northeast-1-private.id
  route_table_id = aws_route_table.ap-northeast-1-private.id
}
...
resource "aws_subnet" "ap-northeast-1-private" {
  vpc_id = aws_vpc.main_vpc.id
